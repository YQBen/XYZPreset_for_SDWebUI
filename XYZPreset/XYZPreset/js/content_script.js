        ...
        li.addEventListener("click", async function () {
            ...
            for (var i = 0; i < allProgram.length; i++) {
                if (Object.keys(allProgram[i]) == this.innerText) {
                    currentIndex = i
                    // alert('solidity');
                    // document.querySelector('#script_txt2img_xyz_plot_x_values > label > textarea').value = 'hello'
                    // return;
                    await sleep(601)
                    await setValue(allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].a, '#script_txt2img_xyz_plot_x_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input')
                    await sleep(601)
                    await setValue(allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].b, '#script_txt2img_xyz_plot_y_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input')
                    await sleep(601)
                    await setValue(allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].c, '#script_txt2img_xyz_plot_z_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input')
                    await sleep(601)
                    document.querySelector('#script_txt2img_xyz_plot_x_values > label > textarea').value = allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].v1
                    await sleep(601)
                    document.querySelector('#script_txt2img_xyz_plot_y_values > label > textarea').value = allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].v2
                    await sleep(601)
                    document.querySelector('#script_txt2img_xyz_plot_z_values > label > textarea').value = allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].v3
                    break
                }
            }
        });
...
async function addbtn() {
    ...
    btn.onclick = async function () {
        ...
        // document.querySelector('#script_txt2img_xyz_plot_x_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input').value = allProgram[currentIndex][Object.keys(allProgram[currentIndex])[0]].v1
    ...
    btn1.onclick = function () {
        if (document.querySelector('#programName').value == '') {
            ...
            var a = document.querySelector('#script_txt2img_xyz_plot_x_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input').value
            var b = document.querySelector('#script_txt2img_xyz_plot_y_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input').value
            var c = document.querySelector('#script_txt2img_xyz_plot_z_type > label > div > div.wrap-inner.svelte-1xsj8nn > div > input').value
            var v1 = document.querySelector('#script_txt2img_xyz_plot_x_values > label > textarea').value
            var v2 = document.querySelector('#script_txt2img_xyz_plot_y_values > label > textarea').value
            var v3 = document.querySelector('#script_txt2img_xyz_plot_z_values > label > textarea').value
    ...
    span.innerText = 'XYZPreset v1.6 by AIL/知筑社'
...
window.onload = async function () {
    ...
    if (window.location.href.indexOf('imdo') != -1 || window.location.href.indexOf('localhost') != -1 || window.location.href.indexOf('127.0.0.1') != -1) {
        ...
        console.log('等待元素出现');
        while (!document.querySelector('#script_list > label > div.wrap.svelte-1xsj8nn > div.wrap-inner.svelte-1xsj8nn > div > input')) {
            await sleep(100)
        }
        console.log('元素已出现');
        var ff = 0
        while (true) {
            //console.log('循环中')
            if (document.querySelector('#script_list > label > div.wrap.svelte-1xsj8nn > div.wrap-inner.svelte-1xsj8nn > div > input').value == 'X/Y/Z plot') {
